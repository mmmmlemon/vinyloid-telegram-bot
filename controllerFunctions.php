...
function deleteitemCommand($chatId){
    ...
    $response = ["Вот полный список всех исполнителей/групп 👩‍🎤 и альбомов 💽. \nЧтобы удалить введите название одного из них 🗑️\n\n"];
...
function showitemsCommand($chatId){
    ...
    if(count($notifications) == 0) {
        ...
        $response = "Список всех исполнителей/групп 👩‍🎤 и альбомов 💽 для проверки ✔\n\n";
        foreach($notifications as $notification){
...
function deleteNotification($chatId, $item){
    ...
    if($check === true){
        $response = "Уведомление <b>{$item}</b> удалено 🗑️\nБольше эта пластинка\исполнитель не будет проверяться.";
        return $response;
    } else {
